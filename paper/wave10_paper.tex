% LEGACY DOCUMENT (historical paper wave10)
...
\hypersetup{
  ...
  citecolor=blue,
  urlcolor=blue,
  pdftitle={Trinity-s3ai: An Active Boundary-Mapping Research Program on H4-Based Standard Model Unification},
  pdfauthor={Anonymous},
}
...
%%  Title block
%% ============================================================
\title{\textbf{Trinity-s3ai: An Active Boundary-Mapping Research Program\\
on H4-Based Standard Model Unification}\\[0.5em]
{\normalsize Four Boundary Theorems for NCG Approaches Founded on the 600-Cell}}
...
precise formulas for SM parameters, we arrive at a \emph{mapped boundary} on direct H4-based unification.

We prove four boundary theorems (BT-1 through BT-4).
\BT{1} establishes that no rational power combination
...
\BT{2} proves, partly via machine-verified Coq lemmas, that no analogue of the
...
\BT{3} shows that the antipodal involution \(v \mapsto -v\) on the 600-cell vertices
...
\BT{4} invokes Schur's lemma for the binary icosahedral group \(\TwoI\)
...
boundary theorems; binary icosahedral group; Coq formalization; Standard Model parameters;
...
and obstruction theorems are documented with the same care as positive ones.
...
results have crystallized into four provable boundary theorems, and six positive structural
...
We emphasize that this is a \emph{constructive} boundary mapping: we do not merely fail to find a
...
Trinity-s3ai provides analogous boundary theorems for the H4 approach.

\subsection{Why publish boundary theorems?}

Boundary theorems are among the most valuable contributions in theoretical physics.
...
cited papers in high-energy physics despite (or because of) being a boundary theorem on vacuum energy.
...
An honest boundary theorem published with full proofs narrows the solution space and guides
...
structural results. Section~\ref{sec:boundary} presents the four boundary theorems with proof sketches.
...
an \emph{exact symmetry} of \(\TwoI\). This observation is central to \BT{3}
(Section~\ref{sec:bt3}).
...
This is the basis for \BT{4} (Section~\ref{sec:bt4}).
...
\(\dagger\dagger\)~Falsified by \({\sim}118\) orders of magnitude (see \BT{1}).
...
However, \BT{3} (Section~\ref{sec:bt3}) shows that the \emph{bulk} operator \(\DF\)
...
this is the starting observation for \BT{3}.
...
\begin{table}[htbp]
  \centering
  \caption{Six positive structural results surviving the boundary theorems.}
  \label{tab:positive}
  \begin{tabular}{cllll}
    \toprule
...
%% ============================================================
%%  Section 5: Boundary theorems
%% ============================================================
\section{Boundary Theorems}
\label{sec:boundary}

We now state and prove the four boundary theorems. All proofs are formalized in
\texttt{BoundaryTheorems.v}. Table~\ref{tab:bt} summarizes the results.

\begin{table}[htbp]
  \centering
  \caption{Summary of four boundary theorems.}
  \label{tab:bt}
  \begin{tabular}{clll}
    \toprule
    Theorem & Statement & Proof method & Status \\
    \midrule
    \BT{1} & Cosmological impossibility & Direct falsification & Planck 2018 data \\
    \BT{2} & No \(\sigma\)-field from H4 & Coq (\(\Qed\) lemmas) & Machine-verified \\
    \BT{3} & Vector-like spectrum & Antipodal symmetry & Analytic + numerical \\
    \BT{4} & Mass hierarchy impossible & Schur's lemma & Rep theory \\
    \bottomrule
  \end{tabular}
\end{table}

\subsection{\BT{1}: Cosmological impossibility}
\label{sec:bt1}

\begin{theorem}[\BT{1}; Cosmological Boundary]
\label{thm:BT1}
...
\subsection{\BT{2}: No \(\sigma\)-field from H4}
\label{sec:bt2}
...
\begin{theorem}[\BT{2}; No \(\sigma\)-field]
\label{thm:BT2}
...
\subsection{\BT{3}: Vector-like fermion spectrum}
\label{sec:bt3}

\begin{theorem}[\BT{3}; Chirality Obstruction]
\label{thm:BT3}
...
\begin{remark}
\BT{3} is \emph{not} the Distler-Garibaldi theorem. The latter applies to E8 as a Lie algebra
and its real forms~\cite{Distler:2009dm}; H4 is not a Lie algebra, and the DG theorem does not
directly apply. \BT{3} is an independent structural argument specific to the antipodal symmetry
...
\subsection{\BT{4}: Mass hierarchy by Schur's lemma}
\label{sec:bt4}

\begin{theorem}[\BT{4}; Mass Hierarchy Obstruction]
\label{thm:BT4}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lll}
    ...
    Boundary mechanism & DG: SL(2,C) centralization & BT-3: antipodal symmetry \\
    Chirality obstruction & adj(E8) self-conjugate & spectrum \(\pm\)-symmetric \\
    Machine formalization & none & \(\sim\)730 \(\Qed\), Coq \\
    Self-assessment & Lisi maintains claim & Project proves own BTs \\
    Key analogy & DG theorem $>$ Lisi claim & BT $>$ Trinity claim \\
    \bottomrule
  \end{tabular}
\end{table}

The critical distinguishing feature of Trinity-s3ai is \emph{self-refutation}: the project
itself derives the boundary theorems, rather than having them imposed externally. This is
...
The results of Sections~\ref{sec:positive} and~\ref{sec:boundary} identify specific obstacles
...
for H4 by \BT{2}. This means the H4 spectral triple, even embedded in the standard NCG SM,
...
\begin{enumerate}
  \item \texttt{proofs/trinity/} --- 37 files covering catalog, spectral action,
    uniqueness, gauge, Higgs, cosmology, and boundary theorems;
...
\begin{itemize}
  ...
  \item BT-1 through BT-4 partial formalizations (\texttt{BoundaryTheorems.v});
...
The six positive structural results (Section~\ref{sec:positive}) survive the boundary theorems.
...
\begin{enumerate}
  \item ``H4 derives the Standard Model'' --- refuted by BT-1 through BT-4 collectively;
  \item ``\(\alpha\) is computable from H4'' --- \(\Qed\) theorem \texttt{alpha\_from\_H4\_refuted}
    in \texttt{RGRunningExtras.v};
  \item ``The 600-cell is chiral'' --- refuted by BT-3;
  \item ``\(D_F\) reproduces SM fermion masses'' --- refuted by BT-4 at \(\sigdist = 5.62\);
  \item All 9 Tier-3 cosmological formulas --- refuted by Planck 2018 and DESI 2024.
\end{enumerate}

\subsection{Implications for NCG programs}

BT-3 implies that \emph{any} finite spectral triple built from the regular representation
...
The specific failure mode of BT-4 (Schur's lemma forcing degenerate multiplets) shows
...
We have presented Trinity-s3ai as a \textbf{boundary-mapping research program} on H4-based
Standard Model unification. After eight waves of machine-verified Coq formalization,
the project has:

\begin{enumerate}
  \item Proved four boundary theorems (BT-1 through BT-4) establishing that H4/600-cell
...
An honest boundary theorem is a scientific contribution. It narrows the solution space,
prevents others from pursuing provably obstructed direct paths, and provides a foundation on which
...
\begin{longtable}{lll}
  ...
  \texttt{BoundaryTheorems.v} & \texttt{BT1\_baryon\_density\_refuted} & Q \\
  \texttt{BoundaryTheorems.v} & \texttt{BT2\_sigma\_field\_impossible} & Ax \\
  \texttt{BoundaryTheorems.v} & \texttt{BT3\_vector\_like\_spectrum} & Q \\
  \texttt{BoundaryTheorems.v} & \texttt{BT4\_mass\_hierarchy\_Schur} & Q \\
