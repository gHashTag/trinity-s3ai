% LEGACY DOCUMENT (v4.0 arXiv submission — historical version)
...
\sin^2\theta_W &= \frac{3\ph^{-6}\pi^2}{e^2} \approx 0.2318 \text{ (MS-bar)}, \quad
    \frac{3}{8\ph} \approx 0.2318 \text{ (on-shell approx.)}, \\
...
\textbf{The $\delta_{CP}$ prediction -- a genuinely risky prediction.} The value $\delta_{CP} = 3/\ph^2 \approx 65.66^\circ$ is in \textbf{5.6$\sigma$ tension} with the NuFit 6.0 global best fit of $\sim$$177^\circ \pm 20^\circ$ (2024). This is not a ``safe'' prediction -- it disagrees with current data. We publish it anyway for three reasons: (1) scientific integrity demands risky predictions, not just post-dictions; (2) the formula $\delta_{CP} = 3/\ph^2$ is mathematically constrained by the $H_4$ structure and cannot be adjusted without breaking the framework; (3) DUNE (40 kt, first data 2028) will render a binary verdict within 3--5 years.

\textit{Historical note:} The $\delta_{CP} = 3/\ph^2$ prediction is \textbf{WITHDRAWN} (post-hoc fit excluded at >5$\sigma$ by NuFit 6.0 + T2K+NOvA 2025). Historical falsification criteria preserved for reference only: If DUNE measures $\delta_{CP} \in [50^\circ, 80^\circ]$, Trinity would have been confirmed at $\geq$3$\sigma$. If DUNE measures $\delta_{CP} > 100^\circ$ or $\delta_{CP} < 30^\circ$, Trinity's PMNS sector is falsified at $\geq$3$\sigma$. The Jarlskog invariant $|J_{\text{Trinity}}| = 0.0327$ agrees with $|J_{\text{exp}}| = 0.0295$ at the 10\% level, suggesting the overall CP violation magnitude is correct even if the phase is tensioned.

DUNE (40 kt, 2029) will measure $\delta_{CP}$ with $\sim$$15^\circ$ precision; regardless of the outcome, the Trinity $\delta_{CP} = 3/\ph^2$ prediction is already **WITHDRAWN** (>5$\sigma$ excluded by current global fits).
...
\subsection{The $\delta_{CP}$ Prediction as a Falsification Test}

The $\delta_{CP} = 3/\ph^2 \approx 65.66^\circ$ prediction deserves special discussion because it carries the highest falsification risk in the framework. We frame it explicitly as a \textit{genuinely risky prediction} for three reasons:

\textbf{(1) It disagrees with current data.} NuFit 6.0 (2024) gives $\delta_{CP} \sim 177^\circ \pm 20^\circ$, placing the Trinity prediction at $5.6\sigma$ tension. Most phenomenological models would adjust their parameters or abandon such a prediction. We do neither -- the formula $\delta_{CP} = 3/\ph^2$ is a fixed consequence of the $H_4$ geometric structure and cannot be modified without breaking internal consistency.

\textbf{(2) It is mathematically constrained.} Unlike the Higgs mass formula $m_H = 4\ph^3 e^2$ (which could in principle acquire a small multiplicative correction), the phase $3/\ph^2$ is an angular coordinate determined by the $H_4$ root system geometry. There is no free parameter to absorb a discrepancy.

\textbf{(3) DUNE renders a binary verdict.} With $\pm 10^\circ$ precision (2028) and $\pm 5^\circ$ (2032), DUNE will either:
\begin{itemize}
    \item \textbf{Historical (superseded):} The $\delta_{CP} = 3/\ph^2$ prediction is WITHDRAWN. Original criterion: Measure $\delta_{CP} \in [50^\circ, 80^\circ]$ $\to$ Trinity confirmed at $\geq$3$\sigma$
    \item Measure $\delta_{CP} > 100^\circ$ or $< 30^\circ$ $\to$ Trinity falsified at $\geq$3$\sigma$
\end{itemize}

\textbf{Honest probability assessment:} $\sim$30\% confirmation, $\sim$60\% falsification, $\sim$10\% unexpected outcome (e.g., new physics, systematic shifts in global fits).

The Jarlskog invariant provides a consistency check: $|J_{\text{Trinity}}| = \sin\theta_{12}\sin\theta_{23}\sin\theta_{13}\cos^2\theta_{12}\cos\theta_{23}\sin\delta_{CP} \approx 0.0327$, which agrees with the experimental $|J_{\text{exp}}| \approx 0.0295$ at the $1.11 \pm 0.04$ level (within 3$\sigma$). This suggests that the \textit{magnitude} of CP violation is correctly captured by the $H_4$ structure even if the \textit{phase} is in tension with current data.
