...
**HiggsOrigins.v** честно отражает эту ситуацию: аксиома `H01_spectral_key_identity` явно маркирует центральный непроверенный шаг. Admitted `H03_h_half_structural` честно признаёт, что структурное тождество для h/2=15 не найдено в алгебраической форме.
