...
(*                                                                            *)
(* ЧЕСТНЫЙ ВЕРДИКТ: Теорема Дистлера–Гарибальди к H4 не применима (доказано  *)
(* структурно). Однако хиральность в Trinity-s3ai на уровне 600-ячейника     *)
(* не достигается автоматически: инволюция v -> -v даёт векторно-подобный    *)
(* спектр. Проблема открыта. Путь (b) через η-инвариант наиболее             *)
(* перспективен математически. Путь (c) — честная оценка текущего состояния. *)
(******************************************************************************)
